...
impl LightMesh {
    ...
    pub fn new(device: &Device, horizontal_count: u32, vertical_count: u32, radius: f32) -> Self {
        if horizontal_count == 0 { panic!("horizontal_count must be >= 1"); }
        if vertical_count < 3 { panic!("vertical_count must be >= 3"); }
        let (vdata, idata) = Self::create_mesh_data(horizontal_count, vertical_count, radius);
    ...
    // I meshed up...
    fn create_mesh_data(horizontal_count: u32, vertical_count: u32, radius: f32) -> (Vec<Vertex>, Vec<u32>) {
        ...
        for r in 1..=horizontal_count {
            ...
            for v in 0..vertical_count {
                ...
                let x = -h_radians.sin() * ring_radius * radius;
                let z = h_radians.cos() * ring_radius * radius;
                vertices.push([x, y, z]);
            }
        }
        vertices.push([0.0, radius, 0.0]);
